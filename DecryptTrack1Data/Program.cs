...
namespace DecryptTrack1Data
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                string decryptedString = Decryptor.DecryptData(KSN, DATA);
                //string decryptedString = Decryptor.DeriveKey(ConversionHelper.ByteArrayToHexString(KSN));
                byte[] hexBytes = Encoding.ASCII.GetBytes(decryptedString);
                string convertedTrack = ConversionHelper.ByteArrayToHexString(hexBytes);
                //Console.WriteLine($"DECRYPTED: {decryptedString}");
                Console.WriteLine($"DECRYPTED: [{convertedTrack}]");
            }
            catch (Exception e)
            {
