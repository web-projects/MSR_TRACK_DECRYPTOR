...
# AUTHRESPONSECODE = [ (0x8A), [0x30,0x30] ]
...
AMOUNT = b'\x00\x00\x00\x01\x01\x00'
DATE = b'\x20\x01\x01'
TIME = b'\x00\x00\x00'
...
def getAnswer(ignoreUnsolicited = True, stopOnErrors = True):
    while True:
        ...
        if uns and ignoreUnsolicited:
            ...
            continue
        if status != 0x9000 and status != 0x9F36 and status != 0x9f22:
...
def processEMV(tid):
    ...
    start_trans_tag = [
        ...
        # [(0x9F,0x41), b'\x00\x01' ], # transaction counter
         [(0x5F,0x2A), b'\x08\x40' ], # currency code
    ...
    continue_tran_tag = [
        ...
        ,[ (0xC2), [0x36, 0x35] ]      # acquirer id
        #,[ (0xDF,0xA2,0x18), [0x00] ] # pin entry style
        #,[ (0xDF,0xA3,0x07), [0x03,0xE8] ] # bit map display
        # ,[ (0xC0), [0x01] ]
        ,OFFLINEPINVERIFY if ISOFFLINE else ONLINEPINVERIFY # card is not blacklisted
    ...
    if args.online == "y":
        ...
        # continue transaction
        conn.send([0xDE, 0xD2, 0x02, 0x00], continue_tpl)
            ...
            if tlv.tagCount(0xE4):
                if args.online == "y":
                    ...
                    while response != "approved" and attempts < args.pinattempts:
                        ...
                        log.log("EMV Response: "+ response)
                        if response != "approved":
                            displayMsg('Invalid emv', True)
                            attempts += 1
                        TC_TCLink.SetProperties(args, log)
                    if response != "approved" and attempts >= args.pinattempts:
                        displayMsg('Emv try limit exceeded', True)
                    removeEMVCard()
                    displayMsg(response, True)
                    conn.send([0xD2, 0x01, 0x01, 0x01])
                    log.log("EMV transaction:", response)
                    return -1
...
if __name__ == '__main__':
    ...
    arg.add_argument( '--custid', dest='custid', default='1152702', type=int,
    ...
    arg.add_argument( '--amount', dest='amount', default='10100', type=int,
